...
for i = 0,game.ReplicatedStorage.GameData.LatestRoom.Value do
						if workspace.CurrentRooms:FindFirstChild(i) and not table.find(nodes,i) then
							local room = workspace.CurrentRooms[i]

							local pathfindNodes = room:FindFirstChild("PathfindNodes")

							if pathfindNodes then
								pathfindNodes = pathfindNodes:Clone():GetChildren()
								room:FindFirstChild("PathfindNodes"):Destroy()
							else
							...
							nodes[#nodes] = pathfindNodes
							elseif workspace.CurrentRooms:FindFirstChild(i) and table.find(nodes,i) then
							continue
							elseif not workspace.CurrentRooms:FindFirstChild(i) and table.find(nodes,i) then
...
function dragHumanoidEntity(entityModel,object,speed,offset)
	...
	loop = RS.Heartbeat:Connect(function(delta)
		if entityModel.Parent and not entityModel:GetAttribute("NoAI") then
			local goalCFrame = startCFrame:Lerp(object.CFrame + offset, alpha)
			entityModel.Humanoid.WalkSpeed = speed
			entityModel.Humanoid:MoveTo(goalCFrame.Position)
			alpha = alpha + delta / relativeSpeed
			if alpha >= 1 then
				loop:Disconnect()
				reachedEvent:Fire()
...
Spawner.Create = function(config)
	...
	local entityModel = not config.Entity.Asset:IsA("Model") and LoadCustomInstance(config.Entity.Asset):Clone() or config.Entity.Asset:IsA("Model") and config.Entity.Asset:Clone()
	...
	if typeof(entityModel) == "Instance" and entityModel.ClassName == "Model" then
		...
		if entityModel.PrimaryPart then
			...
			function entityTable:Run()
				if GetClosetsInRoom(workspace.CurrentRooms[Plr:GetAttribute("CurrentRoom")]) == true then
					-- Nodes
					...
					local startNode = entityTable.Config.Movement.Reversed and workspace.CurrentRooms[game.ReplicatedStorage.GameData.LatestRoom.Value+1].RoomExit or workspace.CurrentRooms[game.ReplicatedStorage.GameData.LatestRoom.Value-1].RoomEntrance

					EntityConnections[entityModel] = {}
					local entityConnections = EntityConnections[entityModel]
					if humanoidEntity == true then
						entityModel:PivotTo(startNode.CFrame + Vector3.new(0, 3.5 + entityTable.Config.Entity.HeightOffset, 0))
					else
						entityModel:PivotTo(startNode.CFrame * CFrame.new(0, 0, startNodeOffset) + Vector3.new(0, 3.5 + entityTable.Config.Entity.HeightOffset, 0))
					end
					...
					if entityTable.Config.Lights.Flicker.Enabled then
						ModuleScripts.ModuleEvents.flicker(workspace.CurrentRooms[ReSt.GameData.LatestRoom.Value], entityTable.Config.Lights.Flicker.Duration)
					end
					...
					for cycle = 1, math.max(math.random(cyclesConfig.Min, cyclesConfig.Max), 1) do
						...
						if cyclesConfig.Type == "Blitz" then
							spawn(function()
								for cycle = 1, math.max(math.random(cyclesConfig.Min, cyclesConfig.Max), 1) do
									...
									--	if humanoidEntity == true then
									--		dragHumanoidEntity(entityModel,nodes[nodeIdx],entityTable.Config.Movement.Speed,Vector3.new(0,3.5+entityTable.Config.Entity.HeightOffset,0))
									--	else
									--		dragBlitz(entityModel,nodes[nodeIdx],entityTable.Config.Movement.Speed,Vector3.new(0,3.5+entityTable.Config.Entity.HeightOffset,0))
									--	end
									--end
									--dragBlitz(entityModel,nodes[currentNodeIdx],entityTable.Config.Movement.Speed,Vector3.new(0,3.5+entityTable.Config.Entity.HeightOffset,0))
						...
						for roomIdx = 0, #nodes, 1 do
						if table.find(nodes,roomIdx) then
						local roomNodes = nodes[roomIdx]
						for nodeIdx = 1,#roomNodes,1 do
						if humanoidEntity == true then
								dragHumanoidEntity(entityModel,roomNodes[nodeIdx],entityTable.Config.Movement.Speed,Vector3.new(0,3.5+entityTable.Config.Entity.HeightOffset,0))
								task.spawn(entityTable.Debug.OnReachNode,nodes[nodeIdx])
							else
								dragEntity(entityModel,roomNodes[nodeIdx],entityTable.Config.Movement.Speed,Vector3.new(0,3.5+entityTable.Config.Entity.HeightOffset,0))
								task.spawn(entityTable.Debug.OnReachNode,nodes[nodeIdx])
							end
						end
						end
						...
						if cyclesConfig.Enabled then
							if cyclesConfig.Type == "Ambush" then
								for roomIdx = #nodes,0, -1 do
						if table.find(nodes,roomIdx) then
						local roomNodes = nodes[roomIdx]
						for nodeIdx = #roomNodes,1,-1 do
						if humanoidEntity == true then
								dragHumanoidEntity(entityModel,roomNodes[nodeIdx],entityTable.Config.Movement.Speed,Vector3.new(0,3.5+entityTable.Config.Entity.HeightOffset,0))
								task.spawn(entityTable.Debug.OnReachNode,nodes[nodeIdx])
							else
					...
					if not entityModel:GetAttribute("NoAI") then
						...
						elseif cyclesConfig.Max <= 1 and cyclesConfig.Type == "Blitz" or cyclesConfig.Max <= 1 and cyclesConfig.Type ~= "Blitz" or cyclesConfig.Max ~= 1 and cyclesConfig.Type == "Blitz" then
							...
							bv.Velocity = Vector3.new(0,0,0)
							unloadSound(entityModel)
							bv.Velocity = Vector3.new(0,-entityModel.PrimaryPart.CFrame.LookVector.Y-100,-entityModel.PrimaryPart.CFrame.LookVector.Z-100)
							game:GetService("Debris"):AddItem(entityModel,5)
							delay(5,function()
								task.spawn(entityTable.Debug.OnDespawned)
							end)
