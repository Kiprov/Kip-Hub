		...
		if entityModel.PrimaryPart then
			...
			function entityTable:Run()
				if GetClosetsInRoom(workspace.CurrentRooms[Plr:GetAttribute("CurrentRoom")]) == true then
					...
					local startNode = nodesLocal[startNodeIndex]
					...
					for cycle = 1, math.max(math.random(cyclesConfig.Min, cyclesConfig.Max), 1) do
						...
						for roomIdx = 0, #nodesLocal, 1 do
						if table.find(nodesLocal,roomIdx) then
						local roomNodes = nodesLocal[roomIdx]
						...
						if humanoidEntity == true then
								...
								task.spawn(entityTable.Debug.OnReachNode,nodesLocal[nodeIdx])
							else
