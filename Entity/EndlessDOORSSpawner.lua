	...
	if typeof(entityModel) == "Instance" and entityModel.ClassName == "Model" then
		...
		if entityModel.PrimaryPart then
			...
			function entityTable:Run()
				if GetClosetsInRoom(workspace.Game.Rooms["Room"..Char.RoomInsideNumber.Value]) == true then
					-- Nodes

					local nodes = {}

					for i = 1,Char.RoomInsideNumber.Value do
						if workspace.Game.Rooms:FindFirstChild("Room"..i) then
							local room = workspace.CurrentRooms["Room"..i]
					...
					spawn(function()
						repeat
							...
							for i = 1,10 do
								...
								for i = 1,Char.RoomInsideNumber.Value do
									if workspace.CurrentRooms:FindFirstChild("Room"..i) then
										local room = workspace.CurrentRooms["Room"..i]
					...
					for cycle = 1, math.max(math.random(cyclesConfig.Min, cyclesConfig.Max), 1) do
						...
						if cyclesConfig.Type == "Blitz" then
							spawn(function()
								for cycle = 1, math.max(math.random(cyclesConfig.Min, cyclesConfig.Max), 1) do
									...
									for i = 1,Char.RoomInsideNumber.Value + 1 do
										if workspace.CurrentRooms:FindFirstChild("Room"..i) then
											local room = workspace.CurrentRooms["Room"..i)
						...
						for nodeIdx = 1, #nodes, 1 do
							if humanoidEntity == true then
								...
								dragEntity(entityModel,nodes[nodeIdx],entityTable.Config.Movement.Speed,Vector3.new(0,3.5+entityTable.Config.Entity.HeightOffset,0))
								task.spawn(entityTable.Debug.OnReachNode,nodes[nodeIdx])
							end
						end
